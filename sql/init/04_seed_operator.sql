...
INSERT INTO app.roles (role_name, description)
VALUES ('Dryer', 'Dryer operator')
ON CONFLICT (role_name) DO NOTHING;

INSERT INTO app.roles (role_name, description)
VALUES ('Kiln', 'Kiln operator')
ON CONFLICT (role_name) DO NOTHING;
...
-- Also give Dryer and Kiln roles the same permissions as Operator
INSERT INTO app.role_permissions (role_id, page_id, can_view, can_add, can_edit, can_delete)
SELECT r.role_id, p.page_id,
       CASE WHEN p.page_key IN ('home','dryer_dashboard','dryer_load','dryer_unload','dryer_readings','setting_entry','setting_transactions','kiln_pushing') THEN TRUE ELSE FALSE END,
       CASE WHEN p.page_key IN ('dryer_load','dryer_unload','dryer_readings','kiln_pushing') THEN TRUE ELSE FALSE END,
       FALSE,
       FALSE
FROM app.roles r
CROSS JOIN app.pages p
WHERE r.role_name IN ('Dryer','Kiln')
ON CONFLICT (role_id, page_id) DO NOTHING;
