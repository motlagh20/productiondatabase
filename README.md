...
**Current phase:** **M5 vertical slice build complete** (2026-08-29). Django + DRF backend (`backend/`) and React 19 + TypeScript + Tailwind v4 frontend (`frontend/`) implement the thin F2→F3→F4→F5→F7 trip spine (Setting load → Kiln push → Kiln exit → Packing → Wagon journey trace). The app DB (`mes_app`) is Django-owned; staging remains a read-only historical source. Dimensions are seeded from staging via `seed_dimensions`. PR [#1](https://github.com/motlagh20/productiondatabase/pull/1) (`m5-slice-build` → `m0-docs`) — 50 files, +5622 lines.
