...
**Current phase:** Historical data load **complete & verified** (4 modules + wagon linking, see [OVERVIEW.md](docs/OVERVIEW.md) + [IMPORT_RUNBOOK.md](docs/IMPORT_RUNBOOK.md)). Master Product & Architecture Specification v0.1 + ADRs 0001–0007 approved for the documentation-first path. **M5 (platform architecture) is in documentation phase** — see [ADR-0007](adr/ADR-0007-application-architecture-m5.md) + [M5_PLATFORM_PLAN.md](docs/M5_PLATFORM_PLAN.md). No application code written yet (ADR-0001 compliant). Ready for M5 doc approval → Django/React build.
...
| `adr/` | Architecture Decision Records — incl. [ADR-0004](adr/ADR-0004-pms-as-design-ux-donor.md) (PMS donor), [ADR-0005](adr/ADR-0005-setting-three-layer-model.md) (Setting 3-layer), [ADR-0006](adr/ADR-0006-dimension-master-redefinition.md) (composite product + cleaned operators), [ADR-0007](adr/ADR-0007-application-architecture-m5.md) (M5 app architecture: Django+DRF/React/PostgreSQL) |
